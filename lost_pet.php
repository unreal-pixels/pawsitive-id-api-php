...
function get_single_item($id) {
  $db_results = query_database("SELECT * FROM LostPet WHERE ID = $id");
  $final_results = array();

  if (mysqli_num_rows($db_results) > 0) {
    while ($row = mysqli_fetch_assoc($db_results)) {
      $id = $row["id"];
      $chat_db_results = query_database("SELECT * FROM Chat WHERE type = 'LOST' AND post_id = $id ORDER BY created_at DESC");
      $chats = array();
...
if ($method === 'GET') {
  ...
  $db_results = query_database("SELECT * FROM LostPet ORDER BY created_at DESC");
  $final_results = array();
